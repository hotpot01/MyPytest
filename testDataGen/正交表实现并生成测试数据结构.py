...
def genTestJsonData(dictdata=1):
    # 参数配置放在这里
    update_start_time = ['168172073800', '166172073800']
    update_end_time=['165172073800', '167172073800']
    page_size = [-1,0,2.5,20,21]
    user_id_type = ["open_id","union_id","user_id","people_admin_id"]

    # 生成原始测试数据 （有序数组）
    cases = OrderedDict([('update_start_time', update_start_time), ('update_end_time', update_end_time), ('page_size', page_size), ('user_id_type', user_id_type)])
...
print(len(c))
# for case in c:
#     print(list(case.items()))
#     print(dict(list(case.items())))
...
for case in c:
    print(list(case.items()))
    print(dict(list(case.items())))
    temp.update({"生成-xxx"+str(n):[
    {
      "desc": "获取人才列表-组合测试",
      "in": dict(list(case.items())),
      "expect": {
        "code": 0,
        "data": {},
        "msg": "ok"
...
print(temp)
with open("temp.josn",'w',encoding="utf-8") as f:
